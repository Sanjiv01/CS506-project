"""Item-item co-occurrence baseline.

For each test playlist with seed songs S, score every candidate song t by
the raw co-occurrence count:

    score(t) = sum over s in S of  #{train playlists containing both s and t}

This is a one-hop neighborhood model with no length normalization, no
inverse-frequency reweighting, and no top-K neighbor cut. It isolates the
co-occurrence signal that the cleaner BM25 / kNN models build on top of.

Implementation: with M = (train_playlist x song) binary matrix and
q = test seed indicator (n_songs binary), the score vector is q * M^T * M.
We compute it batched over test playlists, threaded over batches.
"""
import json
import os
...
print(f"sp_train: {sp_train.shape} nnz={sp_train.nnz:,}")
print(f"sp_test_seed: {sp_test_seed.shape} nnz={sp_test_seed.nnz:,}")
...
# --- Threaded batched scoring ----------------------------------------------
def process_batch(b):
    start = b * BATCH_SIZE
    end = min(start + BATCH_SIZE, n_test_pl)
    batch_codes = np.arange(start, end)

    # (bs, n_train_pl): #seeds the test playlist shares with each train playlist
    overlap = sp_test_seed[batch_codes].dot(sp_train_t)
    # (bs, n_songs): aggregate train playlist songs weighted by overlap
    song_scores = overlap.dot(sp_train).toarray()
...
with open(metrics_path, "w") as f:
    json.dump({"model_tag": MODEL_TAG, "metrics": metrics,
               "config": {"top_n": TOP_N, "batch_size": BATCH_SIZE,
