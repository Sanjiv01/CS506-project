"""Basic kNN — playlist-playlist cosine similarity, no IIF reweighting.

For each test playlist, compute cosine similarity to every train playlist,
keep the top-K neighbors, and rank candidate songs by the sum of similarity
weights of neighbors that contain each song. Cosine is implemented by
L2-normalizing each playlist row before the dot product. The neighbor
aggregation uses the min-max-square reweighting from recommendation_final.py.
"""
import json
import os
...
MODEL_TAG = f"knn_k_{KNN_K}"
print(f"kNN cosine, K={KNN_K}")
...
# --- L2-normalized matrices for cosine -------------------------------------
def cosine_csr(df_in, n_rows):
    agg = df_in.drop_duplicates(subset=["pl_code", "song_code"]).copy()
    agg["w"] = 1.0
    agg["w"] = agg.groupby("pl_code")["w"].transform(
        lambda x: x / np.linalg.norm(x.values)
    )
    return spl.coo_matrix(
        (agg["w"].values.astype(np.float32),
         (agg["pl_code"].values, agg["song_code"].values)),
        shape=(n_rows, n_songs),
    ).tocsr()

sp_A = cosine_csr(train, n_train_pl)              # (n_train_pl, n_songs)
sp_test_seed = cosine_csr(test_seen, n_test_pl)   # (n_test_pl, n_songs)

train_dedup = train.drop_duplicates(subset=["pl_code", "song_code"])
sp_A_const_t = spl.coo_matrix(                    # (n_songs, n_train_pl) binary
...
with open(metrics_path, "w") as f:
    json.dump({"model_tag": MODEL_TAG, "metrics": metrics,
               "config": {"KNN_K": KNN_K, "top_n": TOP_N,
